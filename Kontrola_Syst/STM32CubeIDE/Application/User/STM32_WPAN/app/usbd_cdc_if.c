...
static int8_t CDC_Init_FS(void)
{
  ...
  /* --- TENTO ŘÁDEK JE KLÍČOVÝ --- */
	/* Říká hardwaru: Otevři bránu a čekej na první data z PC */
...
{
	// --- BRUTÁLNÍ TEST: Zvoní pošťák? ---
	APP_DBG("!!! HALO, TADY PRERUSENI! PC NECO POSLALO !!!");
	// ------------------------------------

	// --- TVRDÝ HARDWAROVÝ TEST (Ozvěna) ---
	// Cokoliv přijde, okamžitě pošli zpět do kabelu:
	CDC_Transmit_FS(Buf, *Len);
	// --------------------------------------

	// 1. Zkopírování dat do tvého bufferu pro zpracování
	if (*Len < sizeof(usb_rx_buffer)) {
			memcpy(usb_rx_buffer, Buf, *Len);
			usb_rx_len = *Len;
			UTIL_SEQ_SetTask(1 << CFG_TASK_USB_PROCESS, CFG_SCH_PRIO_0);
	}

	/* 2. KLÍČOVÁ OPRAVA: Musíme hardwaru říct, ať příště píše zase do UserRxBufferFS */
	USBD_CDC_SetRxBuffer(&hUsbDeviceFS, UserRxBufferFS);
	USBD_CDC_ReceivePacket(&hUsbDeviceFS);
